function calculate() {
    ...
    // EMISSION LIMITS (From your Excel)
    let limits = {
        NOx: 0.46,
        PM: 0.01,
        CO: 4,
    ...
    let DEF = totalFuel * 0.05; // from Excel trend (~5%)

    let totalCO2 = 0;

    for (let key in limits) {

        let g_per_km = limits[key] * energy_per_km * scaling;

        let total = g_per_km * distance * fleet;
        let total_kg = total / 1000 * congestion;
    ...
    document.getElementById("summary").innerHTML = `
        ...
        <b>Energy per km:</b> ${energy_per_km.toFixed(2)} kWh/km
    `;
}